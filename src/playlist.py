...
class APIMixin:
    __API_KEY: str = os.getenv('SECRET_KEY')
...
class PlayList(APIMixin):
    ...
    def show_best_video(self):
        """
        Выводит лучшее видедо
        """
        playlist_id = self.playlist_id
        playlist_videos = self.service.playlistItems().list(playlistId=playlist_id, part='contentDetails', maxResults=50).execute()
        video_ids: list[str] = [video['contentDetails']['videoId'] for video in playlist_videos['items']]
